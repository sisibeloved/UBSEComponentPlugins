...
对 LBC mock 来说，每次分配都会创建新的 NVMe namespace，所以这里的 `physical_lba_512b` / `physical_offset_bytes` 表示“这个 namespace 内部的起点”，通常是 0。`ssu-mgr` 仍会按 mock SSU 维度做容量记账，但数据面写入的是新 namespace 内的偏移，不是 mock SSU 池里的累计偏移。
...
**为什么容量不是 512M？**
LBC mock 的资源池容量不再固定为 512 MiB。它会按每个物理 extent 的长度创建底层 namespace；逻辑盘容量看 `$DEV`，物理 namespace 容量看 `query --type logdev` 里的底层设备。需要给 mock SSU 设定测试容量时，在 LBC mock 配置里设置 `resource_bytes`。例如想连续分配 16 个 1 GiB 的单物理盘逻辑盘，单条 mock SSU 资源至少要配置为 `17179869184` 字节。
...
ssu_count=3
resource_bytes=1099511627776
```

> 这份配置**只属于 LBC mock 插件**，别放进 `$LBC_PREFIX`，也不是 Manager 全局配置。

真实快速验证一般不用配置文件，只设置 `LBC_PREFIX`。只有需要把 `/dev`、configfs、日志路径、mock SSU 数量或容量等改成测试值时，才设置 `SSU_LBC_MOCK_CONFIG`。`resource_bytes` 表示每块 mock SSU 暴露给 `ssu-mgr` 的容量，单位是字节。`subnqn` 不能超过 31 个字符，默认值 `nqn.2025-01.io.ssu:m0` 已经满足限制。
