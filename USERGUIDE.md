...
本手册面向**只想使用、不打算读源码**的集成方。读完它，你能用自己的程序或脚本，把 SSU 池化组件跑起来：申请空间 → 挂成 `/dev/ssu/ssuN` 逻辑块设备 → 读写数据 → 解挂载/释放。
...
2. 在你的机器上挂出一个**逻辑块设备 `/dev/ssu/ssuN`**；
3. 你像用普通磁盘一样对 `/dev/ssu/ssuN` 读写；
...
| 内核头（可选） | 若要真正读写 `/dev/ssu/ssuN`，需要构建并加载 `ssu_reqshim.ko` 内核模块 |
...
- `--name`：可选的逻辑盘名称；不传则自动生成 `alloc-N`。传了名称后，返回的 `request_id` 就是这个名称。
...
`allocate` 是**异步**的——它立刻返回一个 `request_id`，底层正在 create+attach。不传 `--name` 时会打印类似 `alloc-0`；传 `--name demo-disk` 时会打印 `demo-disk`，并把同样的内容写入 `/tmp/ssu-rid`。
...
名称只允许字母、数字、`_`、`-`、`.`、`:`，最长 63 个字符；它是分配名/request_id，不是最终块设备节点名。块设备路径仍由 `allocate-result-get` 返回，例如 `/dev/ssu/ssu0`。
...
`query --type logdev` 输出（`/dev/ssu/ssuN → /dev/nvmeXnY` 的映射，真实设备名以本机输出为准）：

```
logdev entries: 1
/dev/ssu/ssu0 local alloc-0 0 536870912 /dev/nvmeXnY 1 0
```
...
`$DEV` 指向的 `/dev/ssu/ssuN` 就是一个普通块设备入口，任何 `open`/`read`/`write` 程序都能用。下面命令会覆盖测试盘上的前 4 MiB 数据，只能对测试设备执行：
...
```bash
sudo ./build-lbc/tools/ubsectl unmount --dev "$DEV"     # 拆 /dev/ssu/ssuN，数据留着
...
ubsectl list                                    # 看资源池
ubsectl mount   --dev /dev/ssu/ssuN --host HOST
ubsectl unmount --dev /dev/ssu/ssuN
ubsectl free    --dev /dev/ssu/ssuN
...
int main()
{
    ...
    req.disk_name             = "demo-disk";             // 可选；NULL = 自动 alloc-N
    ...
    /* ---- 第 4 步：读写 /dev/ssu/ssuN（标准 open/read/write，此处略）---- */
...
| `ssu_api_allocate` | 申请空间，返回 `request_id`（异步）；`req.disk_name` 可指定幂等名称 |
| `ssu_api_allocate_result_get` | 按 `request_id` 取逻辑设备路径 + 物理盘 LBA 明细 |
| `ssu_api_mount(device_path, host)` | 挂成 `/dev/ssu/ssuN` |
...
- 控制面全闭环：`allocate`（create+attach）→ `allocate-result-get` → `mount`（建 `/dev/ssu/ssuN`）→ `unmount`/`free`。
- 数据面 MVP：对 `/dev/ssu/ssuN` 的普通 `read/write`，经 ReqShim 映射到底层物理设备。当前下发到底层设备走普通 block/bio 路径。
