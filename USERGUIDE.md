...
- **业务程序**：链接 `libssu_api`，调 SDK 函数（C/C++）。
...
业务程序直接链接 `libssu_api`，调 C 函数。对应同样的生命周期：
...
int main()
{
    ...
    /* ---- 第 5 步：解挂载 + 释放 ---- */
    ssu_api_unmount(result.device_path);   // 保留数据
    ssu_api_free    (result.device_path);  // 擦除并删 namespace
    return 0;
}
```
