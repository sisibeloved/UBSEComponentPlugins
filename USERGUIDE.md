...
**为什么容量不是 512M？**
LBC mock 的资源池容量不再固定为 512 MiB。它会按每个物理 extent 的长度创建底层 namespace；逻辑盘容量看 `$DEV`，物理 namespace 容量看 `query --type logdev` 里的底层设备。需要给 mock SSU 设定测试容量时，在 LBC mock 配置里设置 `resource_bytes`。
