...
sdk_flow=${2:?userguide sdk flow executable required}
sdk_so=${3:?libubse_ssu_sdk.so path required}
root="/tmp/ssu-lbc-mock-userguide-sdk-$$"
prefix="$root/prefix"
dev_dir="$root/dev"
...
SSU_MGR_SOCKET="$socket" "$sdk_flow" "$sdk_so"
...
grep -q 'mount success allocate_id=alloc-0 host_id=local logical_dev=/dev/ssu/sdk-demo-disk' "$log_file"
grep -q 'unmount success logical_dev=/dev/ssu/sdk-demo-disk allocate_id=alloc-0 host_id=local' "$log_file"
