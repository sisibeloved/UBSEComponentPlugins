...
| 代码元素 | 运行态 | 交付产物 | 归属 |
| ---- | ---- | ---- | ---- |
| SSU API | 用户态 | `libssu_api.so` + 头文件 | 本组件 |
...
```c
// 公共头：ssu_api.h（用户态对外稳定 ABI，libubse 导出）
...
| 产物 | 路径 | 装载方 |
| ---- | ---- | ---- |
| `libssu_api.so`（含 `libubse` SDK）+ 头文件 | `/usr/lib`、`/usr/include` | 业务/管理员链接 |
